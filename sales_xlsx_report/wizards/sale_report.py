...
class CustomerInvoiceReport(models.TransientModel):
    ...
    def generate_report(self):
        ...
        f_name = os.path.join(temp_dir, "invoice_xlsx_report.xlsx")
        ...
        row = col = 0
        sheet.write(row, 0, "Sales Person", cell_format)
        sheet.write(row, 1, "Client Name", cell_format)
        ...
        for partner in invoice_partner_ids:
            ...
            if move_ids:
                sheet.write(
                    row, col, move_ids[0].invoice_user_id.name, cell_format_right
                )
                sheet.write(
                    row, col + 1, move_ids[0].partner_id.name, cell_format_right
                )
        ...
        data = f.read()
        f.close()
        name = "%s.xlsx" % ("ClientWiseReport_" + str(datetime.now() or ""))
        wip_report = self.env["customer.report.xlsx"].create(
            {"file_name": name, "file_data": base64.encodebytes(data)}
        )
        view_id = self.env.ref("sales_xlsx_report.customer_sale_report_wizard_form").id
        return {
            "type": "ir.actions.act_window",
            "res_model": "customer.report.xlsx",
            "target": "new",
            "view_mode": "form",
